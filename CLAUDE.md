...
A Laravel + Inertia (React) application for **Narrae** (brand name decided 2026-09-04, domain narrae.fr, T-143; the brand is a runtime setting, and `racines` stays the technical code name of the folder, the GitHub repo and the Docker Compose project), a French voice-first family-memoir service (the "Remento clone" of the original folder name). Blocks 00 to 03 and 07 to 10 of the roadmap are done and tagged; block 04 is code-complete but not closed: the app runs under Sail, the quality gate is green, CI is wired, the brand is editable in the Filament admin, and the domain model exists — nine tables, the R-4 story state machine with guarded transitions, roles and fine-grained permissions, policies, factories and seeders. The narrator recording page works end to end — a choice screen (voice or film yourself, T-210) then six screens, resumable multipart upload to R2/MinIO, IndexedDB draft, written fallback — but **block 04 stays `◐ en cours` until the real-device spike is run**: `docs/spikes/navigateur.md` holds the protocol (five devices, fifteen scenarios, S11-S15 covering video) and needs an iPhone and an Android over HTTPS. The dossier forbids promising iOS resume before it is proven, so do not close or tag block 04 without it. Code in later blocks does not depend on the spike.

Block 04 added the media layer: `app/Services/Storage/` (`MediaStorage`, `S3MediaStorage`, `FakeMediaStorage`), `app/Support/ObjectKeys.php`, the `recordings` and `client_events` tables, `app/Jobs/` (`ConcatenateSegments`, `ReplicateRecording`), `resources/js/recorder/` (state machine, mime picker, IndexedDB draft, resumable uploader, level meter, wake lock) and the narrator pages. Two invariants matter most: nothing says "votre histoire est enregistrée" before a `HeadObject` per segment confirms the storage holds it, and a Postgres trigger refuses to move an `original_path` once set. **T-210 (2026-09-07) added video**: the narrator picks voice or camera on a `choosing_mode` screen _before_ any permission prompt, `recordings.kind` is derived from the announced container and never from a client field, and `TranscodeRecording` extracts the same MP3 from a filmed take — so transcription, the Fluide rendering, the book and the QR never learn there was a camera. It also derives an MP4, because Chrome Android produces WebM that no iPhone can play. The dossier's out-of-scope list carried "vidéo" until then; reversing it is what bumped the five docs to v2.7 and opened D-11. `sail artisan storage:prepare-local` creates the MinIO buckets — the upload fails without it.

Block 03 added the link infrastructure every narrator and family page will lean on: `app/Services/Tokens/` (`TokenService`, `OtpService`), `app/Support/Links.php` and `app/Support/SensitiveGrant.php`, the `ResolveAccessToken`, `NoStore`, `SecurityHeaders` and `RequireSensitiveGrant` middleware, `app/Logging/RedactTokens.php`, and `app/Services/Sms/`. Bearer tokens are 32 random bytes stored as `sha256` only — `TokenService` is the sole reader of `token_hash`, `Links` the sole builder of a URL, and two tests in `tests/Unit/Tokens/TokenServiceTest.php` fail if either rule is broken. The route pattern `[A-Za-z0-9_-]{43}` rejects a malformed link with a 404 before any query.
...
The repo root is the Laravel project. Alongside it: five French-language product documents at v2.7 in `docs/dossier/`, dated Remento screenshots in `docs/reference/` (git-ignored), and the execution roadmap in `docs/roadmap/`.
...
- **Version discipline**: all five docs share one version (`v2.7 — Septembre 2026`). A substantive change bumps the version everywhere and adds a "Changements vX.Y" sentence under the title of doc 01.
...
Start the environment with `./vendor/bin/sail up -d`; the app answers on `http://localhost:8001` (ports are offset, decision T-34). `sail artisan migrate:fresh --seed` gives an admin account, a demo project with one primary narrator, three family members and five stories in five different states — only one of them visible to the family. `sail artisan storage:prepare-local` creates the three MinIO buckets; presigned uploads fail without them. The canonical commands are in `docs/roadmap/01_CONVENTIONS.md` §6: `sail composer check` for the whole PHP gate, `sail npm run check` and `sail npm run types:check` for the front, `sail npm run test` for Vitest, and — **from the Mac, not from the container** — `E2E_BASE_URL=http://localhost:8001 npx playwright test --workers=1` for end-to-end. Both parts matter: inside the container the browser cannot reach the presigned upload host, so the three recorder specs always fail (T-159); and in parallel the shared initiator session races, which the conventions have forbidden since T-111. PHPStan runs at level 8 and must stay at zero errors.
