...
A product dossier and technical roadmap for **Racines** (working name, brand is a variable), a French voice-first family-memoir service (the "Remento clone" of the folder name). There is **no application code yet, no package manager, no build/test tooling, and no git history**. The repo holds five French-language product documents at v2.3 (September 2026), a folder of dated Remento screenshots, and the execution roadmap in `docs/roadmap/`.
...
Until block 00 is done there is nothing to build or test. The canonical build, lint and test commands are defined in `docs/roadmap/01_CONVENTIONS.md` §6 (`sail composer check`, `sail npm run check`, `sail npm run e2e`). The one mechanical check that already works is scanning for R-11 vocabulary, and it must also be run on the roadmap:
