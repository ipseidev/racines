...
Les textes de consentement ne portent plus la mention `[À VALIDER PAR CONSEIL]` : elle s'affichait à la fin de chaque paragraphe sur `/consentements` et dans le tunnel, c'est-à-dire sous les yeux de quelqu'un à qui l'on demandait son accord (T-242). Les textes sont passés en version 1.1, sans la mention ; la 1.0 reste en base, mot pour mot, parce que `consents` la pointe et que la promesse est de pouvoir réafficher ce qui a été lu. **La relecture par un conseil reste due** : elle est un acte posé dans l'administration (`legal_validated_at`), que `golive:check` vérifie, et non une note en bas d'un paragraphe. Les textes sont rédigés et lisibles ; ils attendent une relecture, pas une rédaction.

**Trois manques de conformité restent, et aucun ne se code** (T-242, mentions légales) :

1. **Le médiateur de la consommation.** L'article L616-1 du code de la consommation oblige un vendeur en ligne à nommer son médiateur, avec son adresse postale et son site. Les CGV (article 13) et les mentions légales disent seulement qu'un recours est possible. Il faut **adhérer** à un médiateur — c'est un abonnement annuel, quelques dizaines d'euros — puis écrire ses coordonnées dans les deux textes. À faire avant la première vente, pas avant le go-live.
2. **Un numéro de téléphone public.** La LCEN (article 6 III-1 a) demande un moyen de contact téléphonique. Le réglage `support_phone` existe, il est vide, et le pied de page n'affiche donc que le courriel. Un numéro suffit — il n'a pas à être une hotline.
3. **L'immatriculation au RCS, si l'activité est commerciale.** Les mentions portent « Registre national des entreprises : SIREN 843 299 751 », vrai dans tous les cas depuis 2023. Si la vente de livres imprimés rend l'activité commerciale, il faut ajouter « RCS Bastia 843 299 751 » — à faire confirmer par le conseil, avec deux autres points qu'il faut lui soumettre : la TVA (l'entreprise est redevable, les prix affichés doivent être TTC et les factures porter la TVA), et le fait que les deux hébergeurs sont des sociétés **américaines** dont les serveurs sont en Europe — la politique de confidentialité doit-elle mentionner des clauses contractuelles types pour eux ?
...
| 18ter | **Nettoyer les questions en attente du projet d'Agate** : trois questions proposées n'ont jamais été ouvertes (10, 12 et 14 septembre) et une quatrième s'y est ajoutée le 20 (T-242). En retirer deux depuis l'administration rend le rythme lisible et rend leurs questions au corpus. À faire avant le lundi suivant, sinon la relance de lien parlera encore d'une question de la semaine passée. | bloc 09 | ☐ |
