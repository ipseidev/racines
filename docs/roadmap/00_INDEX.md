...
| 10 | [Tunnel d'achat, Stripe, cadeau, opt-in narrateur](blocs/B10_tunnel_achat_optin.md) | 09 | ☑ terminé (2026-09-05) — checkpoint §7 joué avec un vrai paiement, 3 défauts de paiement corrigés | `bloc-10-done` |
| 11 | [Back-office support et journal d'audit](blocs/B11_backoffice_audit.md) | 10 | ✅ terminé — checkpoint joué le 2026-09-06 | `bloc-11-done` |
| 12 | [Photos, réponse écrite, contributeurs](blocs/B12_photos_contributeurs.md) | 11 | ✅ terminé — checkpoint joué le 2026-09-06 | `bloc-12-done` |
| 13 | [Livre : book-ready, BAT, PDF, QR, impression](blocs/B13_livre.md) | 12 | ⏸ **en pause** — code complet et rendu éprouvé, checkpoint à moitié joué ; livrable M+12, repris plus tard | `bloc-13-done` |
...
|---|---|---|---|
| D-a | **Le domaine des liens a deux sources qui ne se parlent pas** : les routes lisent la config, `Links` lit le réglage en base. Le changer depuis l'administration — le geste que le dossier présente comme supporté — ferait répondre 404 à **chaque** lien sans que rien ne le signale | T-156, bloc 01 | à trancher : maintenant ou bloc 16 |
| D-b | **Le SMS de réaction ne porte pas le mot du proche**, là où le courriel le porte. Un narrateur joignable par SMS seulement apprend qu'on l'a écouté sans lire ce qu'on lui a dit — et c'est le mot qui donne envie de raconter la suivante | bloc 08 | fondateur (un SMS long coûte deux segments) |
| D-c | **L'option téléphone n'a jamais été exercée par un checkpoint.** Elle a ses tests, pas son passage humain. Depuis T-184 elle s'ajoute aussi **après** l'achat, depuis `/espace/commandes` : le passage humain se fait donc sans refaire un tunnel complet | bloc 10 | fondateur |
...
| **Défaut de couture : l'unité passe, la route ne l'exerce pas** | tous | Quatre en trois jours, tous trouvés par un humain ou en préparant son passage : un bouton d'écoute qui journalisait sans ouvrir (T-182), une correction de texte qui n'avait pas de chemin depuis l'histoire (T-186), un contrôle de type qui court-circuitait l'antivirus sur la route photo alors que l'action annonçait l'inverse (T-187), une couture de micro non exercée (T-178). Le point commun : l'action était testée, **la route ne l'était pas**. Mitigation : tout geste qui traverse une requête a un test qui part de l'URL, pas de l'action |
