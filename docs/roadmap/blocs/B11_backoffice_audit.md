...
- [x] `StoryResource` : fiche (question, état, horodatages, **le texte — mot à mot et rendu courant côte à côte**, écoute de l'enregistrement courant par une route dédiée → `played Recording`), actions : `Masquer` / `Archiver` / `Corbeille` (avec motif, uniquement à la demande du narrateur ou de l'Initiateur·rice, motif obligatoire), `Restaurer`, `Corriger le texte` (→ `EditTranscript`, droit `transcripts.edit`), `Ré-émettre le lien d'enregistrement`. **Aucune action « Valider » ni « Partager ».** La correction existe aussi dans la ressource `Transcripts`, pour le travail par lot ; la fiche sert au signalement, qui part toujours d'une histoire nommée (T-186). Le journal garde la **taille** du changement, pas les deux textes — une entrée d'audit ne se réécrit plus, et y recopier un récit intime en ferait un second endroit où il vit.
