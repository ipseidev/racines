...
- [x] Validation `['file', 'max:20480']` **sans** règle `clamav` **et sans règle de type** : le scan passe par le port `Scanner` appelé dans `AttachPhoto`, et non par une règle de validation. La borne de taille reste — elle protège le scanner — mais le **type** se juge après le scan, sinon un fichier hostile déguisé repart avec « le fichier doit être une image » sans avoir jamais été scanné, et sans laisser de trace (T-187). C'est `Sanitizer` qui refuse ensuite ce qui n'est pas une image lisible, ce qu'il faisait déjà. Un refus de l'antivirus écrit `rejected Photo` au journal inaltérable — nom, taille, empreinte, jamais le contenu. Le paquet prévu ne déclare pas Laravel 13, et une règle de validation ne se double pas (décision T-118). `FakeScanner` reconnaît la chaîne EICAR, coupée en deux dans le code source — un fichier qui la contient en entier est détecté comme un virus par les antivirus des postes de travail, et le dépôt devient impossible à cloner.
...
- [x] `ANTIVIRUS_SCANNER=clamav` par défaut, forcé à `fake` par `phpunit.xml` seulement. Le service `clamav` est déclaré dans `compose.yaml` sur l'image `clamav/clamav:stable-debian` — la variante `stable`, en Alpine, n'existe pas en `arm64` et ne démarrait pas sur un Mac Apple Silicon (T-185). Elle embarque ses signatures : `docker compose up -d clamav`, une minute, et le conteneur est `healthy`. Le service est **sous profil Compose** depuis le 2026-09-07 : `sail up -d` ne le lève plus, parce qu'il gardait 667 Mio de mémoire pour un démon que `laradev` avait déjà débranché en basculant `ANTIVIRUS_SCANNER` sur `fake`. Le nommer suffit à l'activer ; le défaut `clamav` de `.env.example` est inchangé.

**2026-09-07 — le contrôle antivirus est débranché en production (D-12, T-216).** Le démon n'y a jamais été installé et `CLAMAV_HOST` n'y a jamais été posé : il valait `clamav`, le nom du conteneur Sail, qui ne résout pas sur un droplet. Comme le scan refuse tout fichier lorsqu'il ne joint personne, **aucune photo n'a pu être déposée en ligne depuis la mise en service** — le checkpoint du 2026-09-06 avait été joué en local, avec le démon, et ne pouvait pas le voir. Un troisième driver, `off` (`NullScanner`), laisse passer et journalise chaque fichier admis ; `ANTIVIRUS_SCANNER=off` est à poser dans Forge (§1.0 de `05_A_FAIRE_HUMAIN.md`). Ce qui reste du contrôle : le refus de ce qui n'est pas une image lisible, le retrait de l'EXIF, le cloisonnement. Doc 04 §12 et doc 03 P0-6 ne présentent plus l'antivirus comme actif (dossier v2.8).
