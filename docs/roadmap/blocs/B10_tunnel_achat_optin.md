...
- [x] **Ajout du 4 septembre 2026 (T-141)** : la fenêtre de bienvenue de la page d'accueil, une réduction contre une adresse, dans la forme du leader et avec nos règles (case des nouvelles à part, décochée, jamais requise ; code par courriel, jamais à l'écran). `resources/js/components/WelcomeOffer.tsx` (`<dialog>` natif), `POST /offre-de-bienvenue` borné, table `leads`, `ClaimWelcomeOffer`, `WelcomeOfferNotification` ; au récapitulatif, « J'ai un code de réduction » (`ApplyDiscountCode`, `POST`/`DELETE /acheter/code`), application automatique par cookie, coupon Stripe `STRIPE_COUPON_WELCOME` envoyé par identifiant, code brûlé à l'encaissement par `FulfillOrder` ; réglages `welcome_offer_*` du pilote et ressource « Les contacts » dans le back-office. Réduction en pourcentage de toute la commande. Tests : `WelcomeOfferTest`, `DiscountCodeTest`, `PercentTest`, `WelcomeOffer.test.tsx`, `welcomeOffer.test.ts`, `welcome-offer.spec.ts`.
...
- [x] Étape 3 « Le cadeau » : date d'envoi (défaut J+1, max J+90), message personnel (défaut prérempli, modifiable), variante selon `gift-experience`. **`ecard` seule est livrée** : le PDF A6 attend la chaîne d'impression du bloc 13, et le message vocal de l'acheteur attend un enregistreur hors page narrateur — les deux valeurs restent dans le drapeau et la validation (décision T-108). **Depuis T-136 :** date **et heure** (par demi-heure), la forme du cadeau n'est plus demandée (une seule livrée, T-108), pas de message quand on raconte soi-même.
...
- [x] Page `/i/{token}` (`narrator/OptIn.tsx`) : une ouverture (T-232 — le nom seul, « Bonjour {prénom}, », « {Initiateur·rice} vous a offert quelque chose. », « Vos souvenirs, de votre voix, pour les vôtres. », sous douze secondes, `aria-hidden`, la page rendue dessous dès le départ), puis le message personnel (texte ou audio), « Ce que cela veut dire pour vous » (3 phrases), les réglages en clair dans une carte — canal (SMS, courriel ou les deux ; jamais le téléphone opéré, T-234), le champ de contact qui suit le canal et arrive déjà rempli, cadence (hebdo par défaut, « toutes les deux semaines » possible), jour et créneau, forme d'adresse, tous déjà posés par l'Initiateur·rice —, une phrase qui nomme les cinq accords que le bouton donne, deux boutons de même taille : « J'accepte » / « Non merci », puis un accordéon « Vos accords » replié : les cinq accords — les quatre consentements et « sujets sensibles » —, chaque titre ouvrant le texte courant et sa version ; donnés par le bouton et journalisés séparément (T-233 ; les cases séparées ont vécu du 3 au 8 septembre) ; puis un accordéon « Paramètres avancés » replié : les souhaits pour plus tard, « transmettre à ma famille » coché d'avance, référent facultatif — rien n'est écrit tant que le choix proposé n'est pas changé (T-236).
- [x] Acceptation → `AcceptInvitation` : `RecordConsent` × 5, `projects.status = active`, `accepted_at`, `collection_started_at`, fenêtres calculées, `ScheduleNextPrompt` (lendemain au créneau), écran de bienvenue — une pluie légère de confettis aux couleurs de la marque à l'arrivée (T-235) — avec « Ajouter le contact {Marque} » (`/vcard`) et un mot sur les souhaits pour plus tard : le choix fait, ou la politique sans directive, sans rien redemander (T-236 ; le formulaire « Dire mes souhaits maintenant / Plus tard » a vécu du 3 au 8 septembre).
...
**2026-09-05 — point 5 joué par le fondateur, en local, sur le décor `espace@example.test` (T-147).** Résultat : **fonctionnel, mais pas au niveau.** Points 1 et 2 de la procédure (frise et garde de visibilité, lien de la semaine et WhatsApp) : conformes. Point 3, « Écouter comme un proche » : le bouton ne fait que remplacer le contenu de la boîte « lien à coller » plus haut dans la page ; le fondateur s'en est aperçu par hasard et « aurait pensé que ça bug ». Point 4, « Les questions » : fonctionne, mais soixante-cinq cartes à trois liens soulignés chacune, et un bouton « Enregistrer l'ordre » en bas de page, « pas très optimisé ». Verdict général : « tout fonctionne, mais tout semble brut, tout fait trop formulaire basique, et dénote complètement avec la propreté du reste de l'app. Aucune animation, aucune micro-interaction. » L'espace a reçu la palette du 4 septembre (T-133) mais ni le socle de composants ni le mouvement du tunnel d'achat (T-135). Écart consigné, passe de design à décider avant de rejouer le point 5 et de poser le tag.

**2026-09-05 — passe de design de l'espace faite (T-149), point 5 à rejouer.** Les cinq pages et leur mise en page reprennent le socle et le mouvement du tunnel ; « Écouter comme un proche » ouvre la page d'écoute dans un nouvel onglet ; la page des questions montre les cinq prochaines dans l'ordre du moteur et l'ordre part tout seul ; les retours arrivent en toast. La suite Playwright de l'espace couvre les nouveaux gestes (fiche de partage, ordre automatique, écarter puis remettre, confirmation avant rétractation). Le décor est semé et propre : rejouer le point 5, puis les points 3 et 4, avant de poser le tag.
