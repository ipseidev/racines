...
export default function HowItWorks({ price, welcomeOffer, heroSample }: Props) {
    ...
    return (
        <>
            ...
            {welcomeOffer.enabled && (
                <Newsletter discountPercent={welcomeOffer.discountPercent} />
            )}
        </>
    );
}
...
/**
 * L'adresse contre une réduction, en bandeau de bas de page.
 *
 ...
 * ne voit arrête les robots. Le bandeau remplace la fenêtre sur cette
 * page : deux fois la même proposition sur une page ressemble à une relance.
 */
function Newsletter({ discountPercent }: { discountPercent: number }) {
    const t = useT();
        ...
        <section aria-labelledby="newsletter" className="bg-brand-deep">
            <div className="mx-auto grid w-full max-w-6xl gap-8 px-6 py-14 text-[#F7F1E6] lg:grid-cols-[5fr_7fr] lg:items-start lg:gap-16 lg:py-16">
                <div className="flex flex-col gap-3">
                    ...
                    >
                        {t('public.how_it_works.newsletter.title', { amount })}
                    </h2>
