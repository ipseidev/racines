(() => {
    ...
    document.addEventListener(
        ...
        (n) => {
            ...
            requestAnimationFrame(() =>
                requestAnimationFrame(() => {
                    (e.form?.reportValidity(), setTimeout(() => (s = !1), 100));
                }),
            );
        },
        !0,
    ...
    document.addEventListener('alpine:init', () => {
        (window.Alpine.data(
            'filamentSchema',
            ({ livewireId: n, schemaKey: e }) => ({
                handleFormValidationError(i) {
                    i.detail.livewireId === n &&
                        this.$nextTick(() => {
                            let t = this.$el.querySelector(
                ...
                handleClientSideStateReset(i) {
                    i.detail.livewireId !== n ||
                        i.detail.schemaKey !== e ||
                        this.$nextTick(() => {
                            let t = this.$el.querySelectorAll('[autofocus]');
                            for (let r of t)
                                if (
                                    r.offsetParent !== null &&
            ...
            window.Alpine.data(
                ...
                ({ path: n, containerPath: e, $wire: i }) => ({
                    $statePath: n,
                    $get: (t, r) => i.$get(l(e, t, r)),
                    $set: (t, r, o, d = !1) => i.$set(l(e, t, o), r, d),
                    get $state() {
                        return i.$get(n);
                    },
                }),
            ...
            Livewire.interceptMessage(({ message: n, onSuccess: e }) => {
                e(({ payload: i }) => {
                    i.effects?.dispatches?.forEach((t) => {
                        if (!t.params?.awaitSchemaComponent) return;
                        let r = Array.from(
                            n.component.el.querySelectorAll(
                                `[wire\\:partial="schema-component::${t.params.awaitSchemaComponent}"]`,
                            ),
                        ).filter((o) => u(o) === n.component);
                        if (r.length !== 1) {
                            if (r.length > 1)
                                throw `Multiple schema components found with key [${t.params.awaitSchemaComponent}].`;
                            window.addEventListener(
                                `schema-component-${n.component.id}-${t.params.awaitSchemaComponent}-loaded`,
                                () => {
