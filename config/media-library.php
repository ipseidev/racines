...
return [
     ...
     */
    /*
     * Le disque privé, jamais `public` : une photo de famille ne se sert pas
     * par une URL devinable. Les URL passent par `temporaryUrl` et sont
     * régénérées à chaque chargement (bloc 12 §6).
     */
     ...
     */
    // La file `media` : les conversions d'images ne doivent pas retarder
    // l'envoi d'une question, qui vit sur `notifications`.
    ...
    /*
     * Le modèle des envois temporaires de Media Library **Pro**.
     *
     * Nommé par sa chaîne et non par `TemporaryUpload::class` : la version
     * Pro n'est pas installée, et référencer la classe fait échouer l'analyse
     * statique sur un symbole introuvable. La valeur n'est jamais lue sans
     * les composants Pro.
     */
     ...
     */
    /*
     * `imagick` et non `gd` : GD ne lit pas le HEIC, ne sait pas retirer les
     * métadonnées d'un coup, et perd l'orientation. Les seniors envoient des
     * photos prises à l'instant depuis un iPhone — donc du HEIC, souvent
     * tourné, et portant les coordonnées GPS de leur salon.
     */
