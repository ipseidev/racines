...
/**
 * Un enregistrement — la voix seule, ou la voix et le visage (`kind`).
 * L'original est sacré : jamais remplacé.
 *
 * @property string $id
 * @property string $story_id
 * @property RecordingSource $source
 * @property RecordingKind $kind
 ...
 * @property string|null $derived_mp4_path
...
final class Recording extends Model
{
    ...
    protected $attributes = [
        ...
        'kind' => RecordingKind::Audio->value,
        'original_disk' => 'r2',
    ...
    /** @var list<string> */
    protected $fillable = ['source', 'kind', 'original_mime', 'segments', 'device_info'];
    ...
    {
        return $this->kind->isVideo();
    }

    /**
     * Ce qu'on donne à regarder.
     *
     * Le dérivé MP4 d'abord. À défaut l'original, mais **seulement s'il est
     * déjà en MP4** : rendre un WebM à un iPhone n'affiche pas une vidéo,
     * ça affiche un lecteur cassé. Tant que le dérivé n'est pas prêt, mieux
     * vaut la voix, qui elle se lit partout.
     */
    public function playableVideoPath(): ?string
    {
        if (! $this->isVideo()) {
            return null;
        }

        if (is_string($this->derived_mp4_path) && $this->derived_mp4_path !== '') {
            return $this->derived_mp4_path;
        }

        return ObjectKeys::extensionForMime((string) $this->original_mime) === 'mp4'
            ? $this->original_path
            : null;
    }
